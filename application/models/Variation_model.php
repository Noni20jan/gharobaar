...
class Variation_model extends CI_Model
{
    ...
    public function add_variation()
    {
        ...
        foreach ($this->languages as $language) {
            if ($this->input->post('label_lang_' . $language->id, true) == "create your own") :
                $item = array(
                    'lang_id' => $language->id,
                    'label' => $this->input->post('custom-name', true)
                );
            else :
                $item = array(
                    'lang_id' => $language->id,
        ...
        // $temp_path = $this->upload_model->upload_temp_image('size-chart');
        // if (!empty($temp_path)) {
        //     //delete old avatar
        //     // delete_file_from_server($this->auth_user->avatar);
        //     $data["size_chart"] = $this->upload_model->size_chart_image_upload($temp_path);
    ...
    //get product variations last
    public function get_product_variations_last($product_id)
    {
        $this->db->where('product_id', clean_number($product_id));
    ...
    public function add_variation_option($variation_id)
    {
        ...
        $data = array(
            ...
            'sku_code' => $this->input->post('sku-variation', true),
            'package_weight' => $this->input->post('package_weight', true),
            'package_weight_units' => $this->input->post('package_weight_units', true),
            'variation_packed_length' => $this->input->post('variation_packed_length', true),
            'variation_packed_width' => $this->input->post('variation_packed_width', true),
            'variation_packed_height' => $this->input->post('variation_packed_height', true)
        );
    ...
    public function edit_variation_option($option_id)
    {
        ...
        $data = array(
            ...
            'price' => $this->input->post('option_price', true),
            'discount_rate' => $this->input->post('option_discount_rate', true),
            'is_default' => $this->input->post('is_default_edit', true),
            'use_default_price' => $this->input->post('use_default_price', true),
            'no_discount' => $this->input->post('no_discount', true),
            'sku_code' => $this->input->post('sku-variation', true),
            'package_weight' => $this->input->post('package_weight', true),
    ...
    //delete variation option(theme)
    public function delete_variation_option_theme($option_id)
    {
    ...
    //delete variation option(custom-Pulok)
    public function delete_variation_option($option_id)
    {
        ...
        if (!empty($option)) {
            $this->db->where('id', $option->id);
            $result = $this->db->update('variation_options', $data);
            return $result;
        }
        return false;
    }
    ...
    public function get_all_option_count($product_id)
    {
        $this->db->select('*');
        $this->db->from('variations');
        $this->db->join('variation_options', 'variations.id = variation_options.variation_id');
