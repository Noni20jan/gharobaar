...
class Bidding_model extends CI_Model
{
    ...
    public function barter_request($product, $barter_product_id)
    {
        $quantity = $this->input->post('product_quantity', true);
        if (empty($quantity)) {
            $quantity = 1;
        ...
        $data = array(
            ...
            'seller_id' => $product->user_id,
            'buyer_id' => $this->auth_user->id,
            'status' => 'new_barter_request',
            'barter_product_id'=> $barter_product_id  ,
            'price_offered' => 0,
            'price_currency' => '',
            'shipping_cost' => 0,
            'updated_at' => date('Y-m-d H:i:s'),
        ...
        if ($this->db->insert('barter_requests', $data)) {
            return $this->db->insert_id();
        }
        return false;
    }
    ...
    public function submit_barter($quote_request)
    {   
        ...
        if (!empty($quote_request) && $this->auth_user->id == $quote_request->seller_id) {
            ...
            $this->db->where('id', $quote_request->id);
            return $this->db->update('barter_requests', $data);
        }
        return false;
    }

    //accept quote
    public function accept_barter($quote_request)
    {
        if (!empty($quote_request) && $this->auth_user->id == $quote_request->seller_id) {
            $data = array(
                'status' => "barter_accepted",
                'updated_at' => date('Y-m-d H:i:s'),
    ...
    public function complete_barter($quote_request)
    {
        if (!empty($quote_request) && $this->auth_user->id == $quote_request->seller_id) {
            $data = array(
                'status' => "completed",
    ...
    public function reject_barter($quote_request)
    {
        if (!empty($quote_request) && $this->auth_user->id == $quote_request->seller_id) {
            $data = array(
                'status' => "rejected_barter",
                'updated_at' => date('Y-m-d H:i:s'),
    ...
    public function close_barter($quote_request)
    {
        if (!empty($quote_request) && $this->auth_user->id == $quote_request->seller_id) {
            $data = array(
                'status' => "closed_by_seller",
                'updated_at' => date('Y-m-d H:i:s'),

                'deal_closed_by_seller_date' => date('Y-m-d H:i:s')
            );
            $this->db->where('id', $quote_request->id);
            return $this->db->update('barter_requests', $data);
        }
    }
    public function close_barter_by_buyer($quote_request)
    {
        if (!empty($quote_request) && $this->auth_user->id == $quote_request->buyer_id) {
            $data = array(
                ...
                'deal_closed_by_buyer_date' => date('Y-m-d H:i:s')
            );
            $this->db->where('id', $quote_request->id);
            return $this->db->update('barter_requests', $data);
        }
    }
    public function review_barter($quote_request)
    {
        if (!empty($quote_request) && $this->auth_user->id == $quote_request->seller_id) {
            $data = array(
                'status' => "barter_reviewed",
    ...
    public function deal_cancel_barter($quote_request,$cancelled_by)
    {
        if (!empty($quote_request) && $this->auth_user->id == $quote_request->seller_id) {
            $data = array(
                'status' => "deal_cancelled",
                'deal_cancelled_date' => date('Y-m-d H:i:s'),
                'deal_cancelled_by'=>$cancelled_by
    ...
    public function get_barter_request($id)
    {
        $id = clean_number($id);
        $this->db->where('id', $id);
        $query = $this->db->get('barter_requests');
        return $query->row();
    }
    ...
    //get paginated quote requests
    public function get_paginated_barter_requests($user_id, $per_page, $offset)
    {
        $this->db->select('barter_requests.*');
        $this->db->join('products', 'barter_requests.product_id = products.id');
        if ($this->general_settings->membership_plans_system == 1) {
            $this->db->join('users', 'barter_requests.seller_id = users.id AND users.banned = 0 AND users.is_membership_plan_expired = 0');
        }
        $this->db->where('products.status', 1)->where('products.is_draft', 0)->where('products.is_deleted', 0);
        $this->db->where("((products.product_type = 'physical' AND products.stock > 0) OR products.product_type = 'digital')");
        $this->db->where('barter_requests.buyer_id', clean_number($user_id))->where('barter_requests.is_buyer_deleted', 0);
        $this->db->order_by('updated_at', 'DESC')->limit($per_page, $offset);
        return $this->db->get('barter_requests')->result();
    }
    ...
    public function get_barter_requests_count($user_id)
    {
        $this->db->join('products', 'barter_requests.product_id = products.id');
        if ($this->general_settings->membership_plans_system == 1) {
            $this->db->join('users', 'barter_requests.seller_id = users.id AND users.banned = 0 AND users.is_membership_plan_expired = 0');
...
//get barter
 ...
 //get vendor paginated quote requests
 public function get_paginated_vendor_barter_requests($user_id, $per_page, $offset)
 {
     $this->db->select('barter_requests.*');
     $this->db->join('products', 'barter_requests.product_id = products.id');
     $this->db->where('products.status', 1)->where('products.is_draft', 0)->where('products.is_deleted', 0);
     $this->db->where("((products.product_type = 'physical' AND products.stock > 0) OR products.product_type = 'digital')");
     $this->db->where('barter_requests.seller_id', clean_number($user_id))->where('barter_requests.is_seller_deleted', 0);
     $this->filter_quote_requests();
     $this->db->order_by('updated_at', 'DESC')->limit($per_page, $offset);
     return $this->db->get('barter_requests')->result();
 }
 ...
 //get vendor quote requests count
 public function get_vendor_barter_requests_count($user_id)
 {
     $this->db->join('products', 'barter_requests.product_id = products.id');
     $this->db->where('products.status', 1)->where('products.is_draft', 0)->where('products.is_deleted', 0);
     $this->db->where("((products.product_type = 'physical' AND products.stock > 0) OR products.product_type = 'digital')");
 ...
 //get new quote requests count
 public function get_new_barter_requests_count($user_id)
 {
     $this->db->where('seller_id', clean_number($user_id));
     $this->db->where('is_seller_deleted', 0);
 ...
 //delete quote request
 public function delete_barter_request($id)
 {
     $id = clean_number($id);
     $quote_request = $this->get_barter_request($id);
     if (!empty($quote_request)) {
         if ($this->auth_user->id == $quote_request->seller_id || $this->auth_user->id == $quote_request->buyer_id) {
             if ($this->auth_user->id == $quote_request->buyer_id) {
                 $data = array(
 ...
 //delete quote if both deleted
 public function delete_barter_request_if_both_deleted($id)
 {
     $id = clean_number($id);
     $quote_request = $this->get_barter_request($id);
     if (!empty($quote_request)) {
         if ($this->auth_user->id == $quote_request->seller_id || $this->auth_user->id == $quote_request->buyer_id) {
             if ($quote_request->is_buyer_deleted == 1 && $quote_request->is_seller_deleted == 1) {
                 $this->db->where('id', $id);
                 return $this->db->delete('barter_requests');
             }
         }
     }
     return false;
    ...
    public function get_admin_barter_requests_count()
    {
        $this->filter_barter_requests();
        $query = $this->db->get('barter_requests');
        return $query->num_rows();
    }

    //get admin quote requests
    public function get_admin_paginated_barter_requests($per_page, $offset)
    {
        $this->filter_barter_requests();
        $this->db->order_by('created_at', 'DESC');
        $this->db->limit($per_page, $offset);
    ...
    //filter quote requests
    public function filter_barter_requests()
    {
        $status = input_get('status');
        $q = input_get('q');

        if ($status == "new_barter_request" || $status == "pending_barter" || $status == "barter_accepted" || $status == "rejected_barter" || $status == "closed" || $status == "completed") {
            $this->db->where('barter_requests.status', $status);
        }
        if (!empty($q)) {
            $this->db->group_start();
            $this->db->like('barter_requests.product_title', $q);
            $this->db->or_like('barter_requests.id', $q);
            $this->db->group_end();
        }
    }

    //delete admin quote request
    public function delete_admin_barter_request($id)
    {
        if (is_admin()) {
            $id = clean_number($id);
            $quote_request = $this->get_barter_request($id);
