...
class Category_model extends CI_Model
{
    ...
    public function build_query($all_columns = false, $lang_id = null)
    {
        ...
        $this->db->select('(SELECT actionable FROM categories_lang WHERE categories_lang.category_id = categories.id AND categories_lang.lang_id = ' . clean_number($lang_id) . ' LIMIT 1) AS actionable_status');
    ...
    */
    ...
    {
        $this->db->where('lookup_type_id', clean_number($feature_type_id));
        $this->db->order_by('name');
    ...
    public function grp_feature_id($feature_name)
    {
        $this->db->where('lookup_type.lookup_type', $feature_name);
        $query = $this->db->get("lookup_type");
        return $query->row();
    }

    public function get_feature_id($feature_value)
    {
        $this->db->where('lookup_values.lookup_code', $feature_value);
        $query = $this->db->get("lookup_values");
        return $query->row();
    }
    ...
    public function add_category_feature_relation_post()
    {
        ...
        $feature_value = $this->input->post('feature_value', true);
        $grp_feature_id = $this->grp_feature_id($feature_name);
        $feature_id = $this->get_feature_id($feature_value);
        $data = array(
            'grp_feature_id' => $grp_feature_id->id,
        ...
        return $this->db->insert('category_feature', $data);
    }
    public function get_cat_features()
    {


        return $this->db->get('category_feature')->result();
    }
    //add category name
