...
class File_model extends Core_Model
{
    ...
    //upload story image
    public function upload_story_image()
    {
        ...
        if (!empty($temp_path)) {
            ...
            @$this->db->close();
            @$this->db->initialize();
            $this->db->insert('images_story', $data);
            $data = array(
                'image_id' => $this->db->insert_id()
            );
            echo json_encode($data);
    ...
    //check for image in session
    public function check_product_image()
    {
        $modesy_images = $this->get_sess_product_images_array();
    ...
    //add product images
    public function add_sizechart_images($variation_id)
    {
        $modesy_images = $this->get_sess_product_images_array();
        if (!empty($modesy_images)) {
            foreach ($modesy_images as $modesy_image) {
                if (!empty($modesy_image)) {
                    ...
                    //move default image
                    copy(FCPATH . "uploads/temp/" . $modesy_image->img_default, FCPATH . "uploads/size_charts/" . $modesy_image->img_default);
                    delete_file_from_server("uploads/temp/" . $modesy_image->img_default);


                    //add to database
                    $data = array(
                        'size_chart' => "uploads/size_charts/" . $modesy_image->img_default
                    );
                    @$this->db->close();
                    @$this->db->initialize();
    ...
    {
        $key = "story_images_" . $user_id;
        $rows = get_cached_data($key);
        if (empty($rows)) {
            $this->db->where('user_id', $user_id);
            $this->db->order_by('images_story.is_main', 'DESC');
            $query = $this->db->get('images_story');
            $rows = $query->result();
            set_cache_data($key, $rows);
        }
        return $rows;
    ...
    public function get_variation_images_uncached($variation_id)
    {
        $this->db->where('id', $variation_id);
    ...
    //get Story images uncached
    public function get_story_images_uncached($user_id)
    {
        $this->db->where('user_id', $user_id);
        $this->db->order_by('images_story.is_main', 'DESC');
        $query = $this->db->get('images_story');
    ...
    public function get_image_by_variation_id($variation_id)
    {
        $this->db->where('variations.id', $variation_id);
        $query = $this->db->get('variations');
        return $query->row();
    }

    //get story image
    public function get_story_image($image_id)
    {
        $this->db->where('images_story.id', $image_id);
        $query = $this->db->get('images_story');
        return $query->row();
    }
    ...
    public function delete_sizechart_image($variation_id)
    {
    $image = $this->get_image_by_variation_id($variation_id)->size_chart;
    if (!empty($image)) {
    
    delete_file_from_server("uploads/size_charts/" . $image);
    ...
    //delete product image
    public function delete_story_image($image_id)
    {
        $image = $this->get_story_image($image_id);
        if (!empty($image)) {
            if ($image->storage == "aws_s3") {
                $this->load->model("aws_model");
                $this->aws_model->delete_product_object($image->image_default);
    ...
    */
    ...
    //upload story
    public function upload_video_story($user_id)
    {
        $storage = "local";
        if ($this->storage_settings->storage == "aws_s3") {
            $this->load->model("aws_model");
    ...
    //get story video
    public function get_user_story_video($user_id)
    {
        $this->db->where('user_id', $user_id);
        $query = $this->db->get('stories');
        return $query->row();
    ...
    //delete story video
    public function delete_story_video($user_id)
    {
        $video = $this->get_story_video($user_id);
        if (!empty($video)) {
            //delete file
            // if ($video->storage == "aws_s3") {
            //     $this->load->model("aws_model");
            //     $this->aws_model->delete_video_object($video->file_name);
            // } else {
            delete_file_from_server("uploads/videos/" . $video->story);
            // }
            $this->db->where('id', $video->id);
