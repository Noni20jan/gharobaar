...
class Ajax_controller extends Home_Core_Controller
{
    ...
    */
    ...
    public function ajax_search()
    {
        ...
        if (!empty($search_type) && !empty($input_value)) {
            if ($search_type == 'member') {
                $data['result'] = 1;
                $response = '<ul>';
                $members = $this->product_model->search_member_products($input_value);
                if (!empty($members)) {
                    foreach ($members as $member) {
                        $response .= '<li><a href="' . $lang_base_url . $member->slug . '">' . get_product_title($member) . '</a></li>';
                    }
                } else {
    ...
    //ajax search pincode
    public function ajax_search_pincode()
    {
        $lang_base_url = $this->input->post('lang_base_url', true);
        $search_type = $this->input->post('search_type', true);
        $input_value = $this->input->post('input_value', true);
        ...
        if (!empty($search_type) && !empty($input_value)) {
            ...
            $data['result'] = 1;
            $response = '<ul>';
            $products = $this->product_model->search_pincode($input_value);
            if (!empty($products)) {
                foreach ($products as $product) {
                    $response .= '<li><a href="' . $lang_base_url . $product->slug . '">' . get_product_title($product) . '</a></li>';
                }
    ...
    public function select_product_variation_option()
    {
        ...
        if (!empty($variation) && !empty($option)) {
            ...
            if ($variation->use_different_price == 1) {
                ...
                if (empty($price) && empty($discount_rate)) {
                    $price = $product->price;
                    $discount_rate = $product->discount_rate;
                }
                if (empty($price) && !empty($discount_rate)) {
    ...
    */
    ...
    public function send_email()
    {
        ...
        } elseif ($email_type == 'order_cancel_buyer') {
            $this->send_email_order_cancel_buyer();
        } elseif ($email_type == 'order_cancel_seller') {
            $this->send_email_order_cancel_seller();
    ...
    public function send_email_new_order()
    {
        if ($this->general_settings->send_email_buyer_purchase == 1) {
            ...
            if (!empty($order)) {
                ...
                if ($this->general_settings->order_sms_functionality) :
                    //sending order text message 
                    $seller_ids = array();
                    foreach ($order_products as $order_p) {
                        $value = $order_p->seller_id;
                        if (!in_array($value, $seller_ids, true) && !empty($value)) {
                            array_push($seller_ids, $value);
                        }
                    }
                    foreach ($seller_ids as $seller_id) {
                        $seller_mobile = get_user($seller_id)->phone_number;

                        $products = get_order_product_seller_id($seller_id, $order_id);
                        foreach ($products as $product) {
                            if (get_product($product->product_id)->add_meet == "Made to order") {
                                $this->order_model->send_order_text($seller_mobile, "made_to_order_msg", $order->order_number);
                            } else if (get_product($product->product_id)->add_meet == "Made to stock") {
                                $this->order_model->send_order_text($seller_mobile, "made_to_stock_msg", $order->order_number);
                            }
                        }
                    }
                endif;
    ...
    //send email order summary to user
    public function send_email_order_cancel_seller()
    {
        if ($this->general_settings->send_email_buyer_purchase == 1) {
            $order_id = $this->input->post('order_id', true);
            $order_id = clean_number($order_id);
            ...
            if (!empty($order)) {
                ...
                $data = array(
                    ...
                    'to' => $to,
                    'buyer_name' => $buyer_name,
                    'template_path' => "email/email_order_cancel"
                );
                $this->email_model->send_email($data);
                        ...
                        if (!empty($seller)) {
                            if ($seller->send_email_when_item_sold == 1 && !in_array($seller->id, $seller_ids)) {
                                ...
                                $data = array(
                                    ...
                                    'to' => $seller->email,
                                    'seller_name' => $seller_name,
                                    'template_path' => "email/email_order_cancel_by_supplier"
                                );
                                $this->email_model->send_email($data);
                            }
                        }
    ...
    //send email order summary to user
    public function send_email_order_cancel_buyer()
    {
        if ($this->general_settings->send_email_buyer_purchase == 1) {
            $order_id = $this->input->post('order_id', true);
            $order_id = clean_number($order_id);
                        ...
                        if (!empty($seller)) {
                            if ($seller->send_email_when_item_sold == 1 && !in_array($seller->id, $seller_ids)) {
                                ...
                                $data = array(
                                    ...
                                    'to' => $seller->email,
                                    'seller_name' => $seller_name,
                                    'template_path' => "email/email_order_cancel_by_supplier_seller"
                                );
                                $this->email_model->send_email($data);
                            }
                        }
    ...
    //open new shop mail
    public function send_open_shop_mail()
    {
        $this->load->model("email_model");
        $this->session->set_flashdata('submit', "send_email");
        if (!empty($this->auth_user->email)) {
            if (!$this->email_model->send_open_shopmail($this->auth_user->email, $this->auth_user->first_name)) {
