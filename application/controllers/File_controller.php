...
class File_controller extends Home_Core_Controller
{
    ...
    {
        $this->file_model->upload_story_image();
    }
    ...
    /**
     * Get Uploaded Image Session size chart
     */
    public function get_sess_uploaded_image_sizechart()
    {
        $file_id = $this->input->post('file_id', true);
        $modesy_images = $this->file_model->get_sess_product_images_array();
        if (!empty($modesy_images)) {
            foreach ($modesy_images as $modesy_image) {
                if ($modesy_image->file_id == $file_id) {
                    echo '<img src="' . base_url() . "uploads/temp/" . $modesy_image->img_small . '" alt="">' .
                        '<a href="javascript:void(0)" class="btn-img-delete btn-delete-product-img-session-sizeChart" data-file-id="' . $modesy_image->file_id . '"><i class="icon-close"></i></a>';
                    break;
                }
            }
        }
    ...
    /**
     * Get Uploaded story Image
     */
    public function get_uploaded_story_image()
    {
        $image_id = $this->input->post('image_id', true);
        $story_image = $this->file_model->get_story_image($image_id);
        if (!empty($story_image)) {
            echo '<img src="' . get_product_image_url($story_image, 'image_small') . '" alt="">' .
                '<a href="javascript:void(0)" class="btn-img-delete btn-delete-product-img" data-file-id="' . $story_image->id . '"><i class="icon-close"></i></a>' .
                '<a href="javascript:void(0)" class="btn btn-xs btn-secondary btn-is-image-main btn-set-image-main" data-image-id="' . $story_image->id . '" data-product-id="' . $story_image->user_id . '">' . trans("main") . '</a>';
        }
    }
    ...
    public function delete_sizechart()
    {
        $variation_id = $this->input->post('file_id', true);
        $this->file_model->delete_sizechart_image($variation_id);
    }
     ...
     * Delete Image
     */
    public function delete_story_image()
    {
        $image_id = $this->input->post('file_id', true);
        $this->file_model->delete_story_image($image_id);
    }
    ...
    public function upload_video_story()
    {
        $user_id = $this->input->post('user_id', true);
        $this->file_model->upload_video_story($user_id);
        echo $user_id;
    }
    ...
    public function load_video_preview_story()
    {
        $user_id = $this->input->post('user_id', true);
        $data['user'] = $this->auth_model->get_user($user_id);
        $this->load->view('dashboard/product/_story_video_res', $data);
    }
    ...
    //delete video
    public function delete_story_video()
    {
        $user_id = $this->input->post('user_id', true);
        $this->file_model->delete_story_video($user_id);
        $data['user'] = $this->auth_model->get_user($user_id);
        $this->load->view('dashboard/product/_story_video_res', $data);
    }
