...
class Bidding_controller extends Home_Core_Controller
{
    ...
    public function barter_request()
    {
        $product_id = $this->input->post('product_id', true);
        $barter_product_id = $this->input->post('barter_product_id', true);
        $product = $this->product_model->get_active_product($product_id);
        if (!empty($product)) {
            if ($product->user_id == $this->auth_user->id) {
                $this->session->set_flashdata('product_details_error', trans("msg_quote_request_error"));
            ...
            $quote_id = $this->bidding_model->barter_request($product, $barter_product_id);
            if ($quote_id) {
                //send email
                $seller = get_user($product->user_id);
                if (!empty($seller) && $this->general_settings->send_email_bidding_system == 1) {
                    $email_data = array(
                        ...
                        'email_content' => trans("you_have_new_quote_request") . "<br>" . trans("quote") . ": " . "<strong>#" . $quote_id . "</strong>",
                        'email_link' => generate_dash_url("barter_requests"),
                        'email_button_text' => trans("view_details")
                    );
                    $this->session->set_userdata('mds_send_email_data', json_encode($email_data));
                }
    ...
    public function accept_barter()
    {
        $id = $this->input->post('id', true);
        $quote_request = $this->bidding_model->get_barter_request($id);
        if ($this->bidding_model->accept_barter($quote_request)) {
            //send email
            $seller = get_user($quote_request->buyer_id);
            if (!empty($seller) && $this->general_settings->send_email_bidding_system == 1) {
                $email_data = array(
                    'email_type' => 'email_general',
                    'to' => $seller->email,
                    'subject' => "Barter Requests",
                    'email_content' => trans("your_quote_accepted") . "<br>" . trans("quote") . ": " . "<strong>#" . $quote_request->id . "</strong>",
                    'email_link' => generate_dash_url("barter_requests"),
                    'email_button_text' => trans("view_details")
                );
    ...
    public function close_barter()
    {
        $id = $this->input->post('id', true);
        $quote_request = $this->bidding_model->get_barter_request($id);
        if ($this->bidding_model->close_barter($quote_request)) {
            //send email
            $seller = get_user($quote_request->buyer_id);
            if (!empty($seller) && $this->general_settings->send_email_bidding_system == 1) {
                $email_data = array(
    ...
    public function close_barter_by_buyer()
    {
        $id = $this->input->post('id', true);
        $quote_request = $this->bidding_model->get_barter_request($id);
        if ($this->bidding_model->close_barter_by_buyer($quote_request)) {
            //send email
            $seller = get_user($quote_request->buyer_id);
            if (!empty($seller) && $this->general_settings->send_email_bidding_system == 1) {
                $email_data = array(
    ...
    public function review_barter()
    {
        $id = $this->input->post('id', true);
        $barter_id = $this->input->post('barter_product_id', true);
        $p = get_product($barter_id);
        $quote_request = $this->bidding_model->get_barter_request($id);
        if ($this->bidding_model->review_barter($quote_request)) {
            //send email
            $seller = get_user($quote_request->buyer_id);
            if (!empty($seller) && $this->general_settings->send_email_bidding_system == 1) {
                $email_data = array(
                    ...
                    'to' => $seller->email,
                    'subject' => "Barter Requests",
                    'email_content' => "Your barter request is under review",
                    'email_link' => generate_dash_url("barter_requests"),
                    'email_button_text' => trans("view_details")
                );
                $this->session->set_userdata('mds_send_email_data', json_encode($email_data));
            }
            redirect(generate_product_url($p));
        } else {
            $this->session->set_flashdata('error', trans("msg_error"));
        }
        redirect($this->agent->referrer());
    ...
    public function deal_cancel_barter()
    {
        $id = $this->input->post('id', true);
        $quote_request = $this->bidding_model->get_barter_request($id);
        $cancelld_by = $this->auth_user->id;
        if ($this->bidding_model->deal_cancel_barter($quote_request, $cancelld_by)) {
            //send email
    ...
    public function complete_barter()
    {
        $id = $this->input->post('id', true);
        $quote_request = $this->bidding_model->get_barter_request($id);
        if ($this->bidding_model->complete_barter($quote_request)) {
            //send email
            $seller = get_user($quote_request->buyer_id);
            if (!empty($seller) && $this->general_settings->send_email_bidding_system == 1) {
                $email_data = array(
    ...
    public function reject_barter()
    {
        $id = $this->input->post('id', true);
        $quote_request = $this->bidding_model->get_barter_request($id);
        if ($this->bidding_model->reject_barter($quote_request)) {
            //send email
            $seller = get_user($quote_request->buyer_id);
            if (!empty($seller) && $this->general_settings->send_email_bidding_system == 1) {
                $email_data = array(
    ...
    {
        $data["user"] = $this->auth_user;
        $data['title'] = "Barter Request";
        $data['description'] = "Barter Request" . " - " . $this->app_name;
        $data['keywords'] = "Barter Request" . "," . $this->app_name;

        $data['num_rows'] = $this->bidding_model->get_barter_requests_count($this->auth_user->id);
        $pagination = $this->paginate(generate_url("barter_requests"), $data['num_rows'], $this->rows_per_page);
        $data['quote_requests'] = $this->bidding_model->get_paginated_barter_requests($this->auth_user->id, $pagination['per_page'], $pagination['offset']);

        $this->load->view('partials/_header', $data);
        $this->load->view('bidding/barter_requests', $data);
        $this->load->view('partials/_footer');
    }
    ...
    public function delete_barter_request()
    {
        $id = $this->input->post('id', true);
        $this->bidding_model->delete_barter_request($id);
        $this->bidding_model->delete_barter_request_if_both_deleted($id);
    }
}
