    ...
    $("#payout_initiate").submit(function(e) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (len != 0) {
                    for (var i = 0; i < len; i++) {
                        $('#payouts_data').append("<tr><td><input type='checkbox' onchange='checkbox(this," + JSON.stringify(Json_data[i]) + ")'></td><td>" + (j = i + 1) + "</td><td><a href='<?php echo admin_url(); ?>order-details/"+ Json_data[i].order_id +"' class='table-link' target='_blank'>"+ Json_data[i].order_id +"</a></td><td>" + Json_data[i].shop_name + "</td><td>" + Json_data[i].created_at + "</td><td>" + Json_data[i].order_status + "</td><td>" + ((Json_data[i].grand_total_amount) / 100).toFixed(2) + "</td><td>" + ((Json_data[i].shipping_charge_to_gharobaar) / 100).toFixed(2) + "</td><td>" + ((Json_data[i].cod_charge) / 100).toFixed(2) + "</td><td>" + (((Json_data[i].Sup_Shipping_gst / 100) + (Json_data[i].Sup_cod_gst / 100) + (Json_data[i].Sup_subtotal_prd_gst / 100))).toFixed(2) + "</td><td>" + ((Json_data[i].net_seller_payable) / 100).toFixed(2) + "</td></tr>")
                    }
                }
