            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <input type="number" class="form-control" name="day_count"
                        placeholder="<?php echo trans('day_count'); ?>" value="1" min="1"
...
$('#feature_type').on('change', function() {
    ...
    if ($('#feature_type').val() == "GROUP_FEATURE") {
        ...
        $('#feature_value').append(
            '<option value="<?php echo $feature->id; ?>"><?php echo $feature->meaning; ?></option>');
        <?php endforeach; ?>
...
function get_subcategories(category_id, data_select_id) {
    ...
    if (subcategories.length > 0) {
        ...
        var select_tag = '<select class="form-control subcategory-select" data-select-id="' + new_data_select_id +
            '" name="category_id[]" onchange="get_subcategories(this.value,' + new_data_select_id + ');">' +
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                var dropdown =
                    "<div class='dropdown' style='float:none;'><button class='btn  dropdown-toggle btn-select-option btn-custom' type='button' data-toggle='dropdown'>Select an Option<span class='caret'></span></button><ul class='dropdown-menu options-dropdown'>" +
                    "<li><a href=<?php echo admin_url(); ?>product-details/" + Json_data[i]
                    .product_id +
                    "><i class='fa fa-info option-icon'></i>View Details</a></li><li><a href=javascript:void(0); onclick=delete_tagged_item('product_controller/delete_tagging_product'," +
                    feature_id + "," + Json_data[i].product_id +
                    ",'Delete')><i class='fa fa-times option-icon'></i>Delete</a></li><li><a href = <?php echo generate_dash_url('edit_product'); ?>/" +
                    Json_data[i].product_id + "> " +
                    "<i class='fa fa-edit option-icon'></i>Edit</a></li></ul>";

                $('#insert_data').append("<tr><td>" + Json_data[i].product_id +
                    "</td><td> <div class='img-table'><img src=<?php echo base_url(); ?>uploads/images/" +
                    Json_data[i].image_small + "> + </div></td><td>" +
                    '<a href="<?php echo base_url(); ?>' + Json_data[i].slug +
                    '"target="_blank" class="table-link">' + Json_data[i].title + "</td><td>" +
                    Json_data[i].name + "</td><td>" + Json_data[i].stock + "</td><td>" +
                    Json_data[i].created_at + "</td><td>" + dropdown + "</td></tr>");
            }
            var table = $('#example').DataTable({
