...
$productss = get_user_barter_products($this->auth_user->id);
// foreach($pooja as $po){
//     var_dump(get_product_details_by_id($po->id)->title);
// }
?>
<div class="row">
    <div class="col-12">
...
<div class="row">
    <div class="col-12">
        ...
        <h3 class="product-title"><?= html_escape($product_details->title); ?></h3>
        ...
        <div class="row-custom price">
            ...
            <button class="btn btn-contact-seller" onclick="open_modal_request_for_barter()"><i class="icon-envelope"></i> Request For Barter</button>
            &nbsp;&nbsp;
...
<div class="modal fade" id="knowmore11" role="dialog">
    <div class="modal-dialog modal-dialog-centered modal-send-message" role="document">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <label class="control-label">You can only barter those products whose price is in 10% price range of
                    barter product</label>
                <br>

                <br> <b><label class="control-label">Select Your Product</label></b>
                <select name="barter_product_id" class="form-control" id="barter_product_id" onchange="get_variations()">
                    <option value="" selected disabled>Select Product</option>
                    <?php
                    $productprice = ($product->listing_price) / 100;
                    foreach ($productss as $item) {

                        $barter_product = ($item->listing_price) / 100;
                        //     var_dump($item);
                        //    var_dump($productprice);
                        //    var_dump($barter_product);
                        if ($barter_product <=  $productprice + (0.1 * $productprice) && $barter_product >=  $productprice -  (0.1 * $productprice)) {
                            //  echo "hello";
                            echo "<option value=" . $item->id . ">" . get_product_details_by_id($item->id)->title . "</option>";
                        }
                    }
                    ?>
                </select>
                <br>
                <div id="product_other_details">
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-md btn-custom" onclick="make_barter_request()">Submit</button>
            </div>
        </div>
...
<script>
    ...
    //     console.log("click");
    // });
    function make_barter_request() {
        console.log("click");
        var base_url = '<?php echo base_url() ?>';
        var product_id = <?php echo $product->id ?>;
        var barter_product_id = $('#barter_product_id option:selected').val();
        var data = {
        ...
        if (product_id && barter_product_id) {

            $.ajax({
                url: base_url + "bidding_controller/barter_request",
                method: "POST",
                data: data,
                success: function(data) {
                    if (data != "") {
                        $(knowmore11).hide();
                        alert("Barter request for this Item succesfully sent");
...
<script>
    $("#barter_product_id").change(function() {

        $("#product_other_details").show();
    }).change();
</script>

<script>
    function open_modal_request_for_barter() {
        $('input[name^="variation"]').each(function() {
            if($(this)[0].checked){
                $("#knowmore11").modal("show");
                $this[0].setCustomValidity('');
            }
            $this[0].setCustomValidity('please select variation');
        })
    }
</script>
<script>
function get_variations()
{
    ...
    console.log("click");
        ...
        if (barter_product_id) {

            $.ajax({
                url: base_url + "bidding_controller/get_variations",
                method: "POST",
                data: data,
                success: function(data) {
                    if (data != "") {
